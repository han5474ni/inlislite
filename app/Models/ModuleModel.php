...
class ModuleModel extends Model
{
    ...
    protected $validationRules = [
        ...
        'module_type' => 'required|in_list[application,database,utility]',
    ...
    protected $validationMessages = [
        ...
        'module_type' => [
            'required' => 'Tipe modul harus dipilih',
     ...
     * Get modules by type
     */
    public function getByType($moduleType, $status = 'active')
    {
        return $this->where('module_type', $moduleType)
                   ->where('status', $status)
                   ->orderBy('sort_order', 'ASC')
                   ->orderBy('created_at', 'DESC')
                   ->findAll();
    ...
    public function getStatistics()
    {
        ...
        // Total modules
        $stats['total_modules'] = $this->where('status', 'active')->countAllResults();
        
        // Application modules
        $stats['app_modules'] = $this->where('module_type', 'application')
                                    ->where('status', 'active')
                                    ->countAllResults();
        
        // Database modules
        $stats['db_modules'] = $this->where('module_type', 'database')
                                   ->where('status', 'active')
                                   ->countAllResults();
        
        // Utility modules
        $stats['utility_modules'] = $this->where('module_type', 'utility')
                                        ->where('status', 'active')
                                        ->countAllResults();
    ...
    protected function afterFind($data)
    {
        ...
        try {
            ...
            log_message('error', 'ModuleModel::afterFind - Exception: ' . $e->getMessage());
        }
